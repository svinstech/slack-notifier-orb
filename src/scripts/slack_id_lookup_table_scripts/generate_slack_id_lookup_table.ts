...
async function main() {
    const notifierBotToken:string = process.argv[2] ?? process.env.SLACK_BOT_TOKEN // First argument must be the notifierBotToken environment variable saved to this CircleCI project.
    ...
    const slackUserInfoFilePath:string = 'slackUserInfo.json',
        ...
        lookupTableFilePath:string = lookupTableFileName ?? 'slackIdLookupTable.txt'
